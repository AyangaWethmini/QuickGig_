...
class userReport
{
    ...
    public function getUserDetailsOrg($accountID)
    {
        $query = 'SELECT a.accountID, a.email, p.planName, i.orgName as fname
              FROM account a
              JOIN plan p ON p.planID = a.planID JOIN organization i ON a.accountID = i.accountID
              WHERE a.accountID = :accountID';
